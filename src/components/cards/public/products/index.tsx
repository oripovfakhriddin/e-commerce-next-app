...
const PublicProductsCard = ({ data, loading }: { data: Products } & { loading: boolean }) => {
  ...
  return (
    <div className='card__box__container'>
      <div className='product__image__box'>
        {loading ? <Skeleton className="product__card__skleton" variant="rectangular" /> : <Image src={data?.image?.url ? data?.image?.url : "https://c8.alamy.com/comp/2ATEFRH/mission-failed-text-on-red-round-stamp-2ATEFRH.jpg"} alt={data?.title ? data?.title : "Ma'lumot topilmadi!"} fill />}
        {typeof data?.category === "object" ? <p>{loading ? <Skeleton height={25} width={90} /> : data?.category?.name}</p> : null}
      </div>
      <div className="product__content__box">
        {loading ? <Skeleton /> : <h3>{data?.title ? data?.title : "Ma'lumot topilmadi!"}</h3>}
        ...
        {loading ? <Skeleton /> : <p>{data?.description ? data?.description : <span style={{ color: "red" }}>Ma'lumot topilmadi</span>}</p>}
      </div>
      <div className="product__action__box">
        ...
        {toggleInCart ?
          ...
          :
          <div>
            <Button onClick={() => { controlProductInCart(data) }} className='add__cart' disabled={loading} >Savatga qo'shish</Button>
          </div>
